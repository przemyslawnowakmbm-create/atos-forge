...
```bash
INIT=$(node ~/.codex/forge/forge-cli/bin/forge-tools.cjs init execute-phase "${PHASE}")
```
...
**Trigger:** UI code violates accessibility or quality standards from `@~/.codex/forge/forge-cli/references/ui-ux-quality.md`
...
```bash
AUTO_CFG=$(node ~/.codex/forge/forge-cli/bin/forge-tools.cjs config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**See @~/.codex/forge/forge-cli/references/checkpoints.md**
...
**Use template:** @~/.codex/forge/forge-cli/templates/summary.md
...
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs state update-progress

# Record execution metrics
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs state record-metric \
  --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
  --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"

# Add decisions (extract from SUMMARY.md key-decisions)
for decision in "${DECISIONS[@]}"; do
  node ~/.codex/forge/forge-cli/bin/forge-tools.cjs state add-decision \
    --phase "${PHASE}" --summary "${decision}"
done

# Update session info
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs state record-session \
  --stopped-at "Completed ${PHASE}-${PLAN}-PLAN.md"
```

```bash
# Update ROADMAP.md progress for this phase (plan counts, status)
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs roadmap update-plan-progress "${PHASE_NUMBER}"

# Mark completed requirements from PLAN.md frontmatter
# Extract the `requirements` array from the plan's frontmatter, then mark each complete
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs requirements mark-complete ${REQ_IDS}
```
...
```bash
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs state add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
node ~/.codex/forge/forge-cli/bin/forge-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
