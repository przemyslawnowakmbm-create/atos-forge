...
> **Note:** Planning methodology is in `agents/forge-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/atos-forge/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/forge:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/atos-forge/references/tdd.md` for TDD plan structure.
...
See `~/.claude/atos-forge/templates/user-setup.md` for full schema and examples
...
See `~/.claude/atos-forge/workflows/verify-phase.md` for verification logic.
