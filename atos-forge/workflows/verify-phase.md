...
<required_reading>
@~/.claude/atos-forge/references/verification-patterns.md
@~/.claude/atos-forge/templates/verification-report.md
</required_reading>
...
Use forge-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ~/.claude/atos-forge/bin/forge-tools.cjs frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ~/.claude/atos-forge/bin/forge-tools.cjs roadmap get-phase "${phase_number}" --raw)
```
...
Use forge-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ~/.claude/atos-forge/bin/forge-tools.cjs verify artifacts "$plan")
...
Use forge-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ~/.claude/atos-forge/bin/forge-tools.cjs verify key-links "$plan")
...
See ~/.claude/atos-forge/templates/verification-report.md for complete template.
</step>
