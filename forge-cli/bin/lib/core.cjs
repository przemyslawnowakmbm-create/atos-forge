...
function getForgeRoot() {
  // FORGE_HOME env var takes priority (e.g. FORGE_HOME=/path/to/forge-cli)
  if (process.env.FORGE_HOME) return process.env.FORGE_HOME;
  // Default: go up 3 levels from forge-cli/bin/lib/core.cjs → parent of forge-cli/
  // bin/lib/core.cjs → bin/lib → bin → forge-cli → ~/.claude/
