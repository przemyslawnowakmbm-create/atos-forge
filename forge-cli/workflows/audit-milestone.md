...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node ~/.claude/forge-cli/bin/forge-tools.cjs resolve-model forge-integration-checker --raw)
```
...
node ~/.claude/forge-cli/bin/forge-tools.cjs phases list
```
...
PHASE_INFO=$(node ~/.claude/forge-cli/bin/forge-tools.cjs find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/forge-cli/bin/forge-tools.cjs summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
