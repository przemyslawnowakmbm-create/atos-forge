...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs validate health $REPAIR_FLAG
```
...
2. **Missing .forge/config.json** → Copy from template: read `forge-cli/templates/config.json`, write to `.forge/config.json`
...
6. **Missing dashboard** → Run `node forge-cli/bin/forge-tools.cjs graph visualize` (if graph.db exists)
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs validate health
```
