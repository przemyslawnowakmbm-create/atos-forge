...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init quick "$DESCRIPTION")
```
...
CURRENT_PHASE=$(node ~/.claude/forge-cli/bin/forge-tools.cjs state-snapshot | jq -r '.current_phase // empty')
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
