...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init phase-op "${target}")
```
...
```bash
RESULT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), forge-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs phase remove "${target}" --force)
```
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
