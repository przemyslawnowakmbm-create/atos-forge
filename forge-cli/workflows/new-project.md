...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init new-project)
```
...
```bash
mkdir -p .planning
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "docs: initialize project" --files .planning/PROJECT.md
```
...
**Determine milestone context** before spawning agents:

Check if this is greenfield or subsequent milestone:
- If no "Validated" requirements in PROJECT.md → Greenfield (building from scratch)
- If "Validated" requirements exist → Subsequent milestone (adding to existing app)

Use this context to fill in the `<milestone_context>` blocks when running the shared research pipeline:
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```

## 8. Create Roadmap

Spawn forge-roadmapper agent with new-project context:
...
**Note (auto mode):** Skip approval gate — auto-approve and commit directly.

**Commit roadmap (after approval or auto mode):**

```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
