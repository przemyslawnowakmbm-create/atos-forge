...
@~/.claude/forge-cli/references/git-integration.md
...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init execute-phase "${PHASE}" --include state,config)
```
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs phases list --type summaries --raw
...
See `~/.claude/forge-cli/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/forge-cli/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/forge-cli/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/forge-cli/templates/summary.md`.
...
node ~/.claude/forge-cli/bin/forge-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/forge-cli/bin/forge-tools.cjs state update-progress

# Record execution metrics
node ~/.claude/forge-cli/bin/forge-tools.cjs state record-metric \
...
node ~/.claude/forge-cli/bin/forge-tools.cjs state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/forge-cli/bin/forge-tools.cjs state add-blocker "Blocker description"
```
</step>
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs requirements mark-complete ${REQ_IDS}
```
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "" --files .planning/codebase/*.md --amend
```
</step>
