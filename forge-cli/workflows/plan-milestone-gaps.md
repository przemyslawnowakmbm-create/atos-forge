...
PHASES=$(node ~/.claude/forge-cli/bin/forge-tools.cjs phases list)
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
