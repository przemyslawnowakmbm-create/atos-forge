...
@~/.claude/forge-cli/references/session-continuity.md
...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
```bash
PHASE_INFO=$(node ~/.claude/forge-cli/bin/forge-tools.cjs roadmap get-phase "${PHASE}")
```
...
```bash
PHASE_DESC=$(node ~/.claude/forge-cli/bin/forge-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ~/.claude/forge-cli/bin/forge-tools.cjs state-snapshot)
...
if [ "$GRAPH_AVAILABLE" = "true" ]; then
  ...
  GRAPH_RAW=$(node ~/.claude/forge-cli/bin/forge-tools.cjs graph status 2>/dev/null || echo "{}")
