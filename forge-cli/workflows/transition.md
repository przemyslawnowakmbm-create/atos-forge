...
```bash
TRANSITION=$(node ~/.claude/forge-cli/bin/forge-tools.cjs phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ~/.claude/forge-cli/bin/forge-tools.cjs progress bar --raw)
```
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs config-set workflow.auto_advance false
```
