...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init map-codebase)
```
...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "docs: map existing codebase" --files .planning/codebase/*.md
```
