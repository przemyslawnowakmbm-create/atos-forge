...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init progress --include state,roadmap,project,config)
```
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node ~/.claude/forge-cli/bin/forge-tools.cjs summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node ~/.claude/forge-cli/bin/forge-tools.cjs progress bar --raw)
```
