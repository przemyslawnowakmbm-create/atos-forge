...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init verify-work "${PHASE_ARG}")
```
...
```bash
node "$HOME/.claude/forge-cli/bin/uat-run-and-eval.cjs" --command "{command}" --expected "{expected}" --cwd "$(pwd)"
```
...
Commit the UAT file:
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @~/.claude/forge-cli/workflows/diagnose-issues.md
