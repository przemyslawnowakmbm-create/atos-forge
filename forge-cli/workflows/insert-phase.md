...
```bash
INIT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node ~/.claude/forge-cli/bin/forge-tools.cjs phase insert "${after_phase}" "${description}")
```
