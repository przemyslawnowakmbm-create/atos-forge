...
if [ -f ./.claude/forge-cli/VERSION ]; then
  cat ./.claude/forge-cli/VERSION
  echo "LOCAL"
elif [ -f ~/.claude/forge-cli/VERSION ]; then
  cat ~/.claude/forge-cli/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
...
```bash
npm view forge-cli version 2>/dev/null
```
...
- `forge-cli/` will be wiped and replaced
...
[View full changelog](https://github.com/forge-team/forge/blob/main/CHANGELOG.md)
```
</step>
