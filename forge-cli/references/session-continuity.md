...
```bash
node ~/.claude/forge-cli/bin/forge-tools.cjs ledger log-preference "User preference text"
```
...
node ~/.claude/forge-cli/bin/forge-tools.cjs ledger read

# Just the state summary (for quick checks)
node ~/.claude/forge-cli/bin/forge-tools.cjs ledger state
```
