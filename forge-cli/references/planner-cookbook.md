...
Reference material for `forge-planner`. Contains worked examples, specificity tables, and depth calibration data extracted to reduce prompt size.

---

## Checkpoint XML Templates
...
<task id="N" type="auto">
  ...
  <action>
    ...
    If no test framework exists, set one up first (see @~/.claude/forge-cli/references/tdd.md <framework_setup>).
  </action>
...
## Task Anatomy Examples (Good vs Bad)
...
## Depth Calibration Tables

### Phase Depth by Plans
...
## Worked Example: Goal-Backward (Chat Interface)

**Phase goal:** "Working chat interface"
...
**Step 3 — Required Artifacts:** "User can see existing messages" requires:
- Message list component (renders Message[])
- Messages state (loaded from somewhere)
- API route or data source (provides messages)
- Message type definition (shapes the data)

**Step 4 — Required Wiring:** Message list component wiring:
...
| Missing wiring | Listing components without connections | "Chat.tsx fetches from /api/chat via useEffect on mount" |

---

## Worked Example: Checkpoint Anti-Patterns
...
## Codebase Context Loading by Phase Type
...
**Checkpoint usage distribution:** human-verify (90%), decision (9%), human-action (1%).

---

## Revision Mode: Dimension Strategy Table
...
## PLAN.md Full Structure Template
...
has_tests: true             # REQUIRED — true/false with reason if test-exempt
...
### Gap Closure Mode — Complete When
...
Gap closure plan frontmatter extras:
```yaml
