# Forge — Internal Developer Reference

<!-- @-imported by workflows that need full API details. Not loaded in every conversation. -->
...
## 16-Layer Verification Engine — Full API
...
Layer sub-details:
 2. TYPE/COMPILE — Broad tsconfig.json discovery: cwd → parent dirs → src/ → packages/* (monorepo).
 ...
 6. TESTS — graph-identified test files for changed code (getContextForTask → testFiles).
    Override: `verification.test_command` in config.
 8. CONTRACT — Code↔YAML drift: detects phantom exports, undeclared exports, stale/undeclared endpoints.
...
10. ARCHITECTURAL — Reads .planning/codebase/ARCHITECTURE.md and CONVENTIONS.md.
    ...
    Enable via: verification.layers.architectural = true. Expensive (LLM call) — use deliberately.
13. COVERAGE — Uses: `collectCoverage(cwd, opts)` from forge-verify/coverage.js.
    Enable via: `verification.layers.coverage = true`. Config: `verification.coverage` section for thresholds.
14. ENTROPY — Uses: `computeEntropy(cwd, opts)` from forge-verify/entropy.js.
    Pass condition: aggregate health !== 'red'. Enable via: `verification.layers.entropy = true`.
15. REGRESSION — Uses: `runRegression(cwd, opts)` from forge-verify/regression.js.
    Enable via: `verification.layers.regression = true`.
...
Configuration in .forge/config.json or .planning/config.json (verification section):
  layers (per-layer boolean toggles: structural, type_check, interface_contracts, dependency_analysis,
    key_links, tests, behavioral, contract, semantic, architectural, browser, mutation,
    coverage, entropy, regression), auto_fix (true/false), max_fix_loops,
...
Additional exports: findTsConfig(cwd), loadVerificationConfig(cwd), layerContract (lazy), layerArchitectural, LAYER_NAMES
...
Auto-fixable: type errors (L2), missing imports (L4), assertion mismatches (L6), interface breaks (L3), syntax issues (L1), contract drift (L8).
Not auto-fixable: behavioral failures (L7) → escalate to human.
...
Wave integration:
- verifyAfterWave(opts) — lighter check (layers 0-4), max 2 loops, after each wave
- verifyFull(opts) — all 16 layers, max 3 loops, after all waves complete
