...
```bash
PHASE_INFO=$(node ~/.claude/forge-cli/bin/forge-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/forge-cli/bin/forge-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/forge-cli/bin/forge-tools.cjs find-phase "${PHASE}" --raw)
```
